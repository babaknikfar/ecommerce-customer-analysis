...
def top_products(df: pd.DataFrame, n: int = 10) -> pd.DataFrame:
    ...
    logger.info(f"Top product: {top_products.index[0]} with revenue {top_products.iloc[0]['total_revenue']:,.2f}")
...
def country_analysis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Top country: {country_stats.index[0]} with {country_stats.iloc[0]['revenue_share_pct']:.1f}% of revenue")
