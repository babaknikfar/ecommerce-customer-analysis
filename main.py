...
def main() -> None:
    ...
    logger.info(
        f"Found missing values in CustomerID: {profile.missing_values['CustomerID']:,}"
    )
    logger.info(f"Found {profile.negative_prices:,} rows with negative prices")
    logger.info(
        f"Date range: {profile.date_range[0].date()} to {profile.date_range[1].date()}"
    )
    ...
    logger.info(
        f"Cleaned data: {df_cleaned.shape[0]:,} rows and {df_cleaned.shape[1]} columns"
    )
    ...
    print(
        f"Time period: {df_cleaned['InvoiceDate'].min().date()} to {df_cleaned['InvoiceDate'].max().date()}"
    )
    print(
        f"Top product: {top_10_products.index[0]} (£{top_10_products.iloc[0]['total_revenue']:,.2f})"
    )
    print(
        f"Top country: {country_stats.index[0]} (£{country_stats.iloc[0]['total_revenue']:,.2f})"
    )
