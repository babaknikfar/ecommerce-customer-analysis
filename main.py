...
from ecommerce_analysis.data_export import (
...
def main() -> None:
    ...
    report_path = export_summary_report(df_cleaned)
    
    logger.info(f"\nExported files:")
    for key, path in exported_files.items():
        logger.info(f"  - {key}: {path}")
    logger.info(f"  - summary_report: {report_path}")
    ...
    print(f"  - Cleaned data: data/processed/cleaned_transactions.csv")
    print(f"  - Analysis results: reports/analysis_results.xlsx")
    print(f"  - Summary report: reports/summary_report.txt")
    print("=" * 60)
